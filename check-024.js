...
async function run() {
  ...
  console.log("Looking for name/reference matching 024:")
  products.forEach(p => {
    const nameFr = p.name?.fr || p.name;
    const slug = productSlug(nameFr || '', p.reference);
    if (slug.includes('024') || (nameFr && nameFr.includes('024'))) {
      console.log(`MATCHED: ID: ${p._id} | Name: ${nameFr} | Ref: ${p.reference} | Slug: ${slug}`)
    }
  })
}
